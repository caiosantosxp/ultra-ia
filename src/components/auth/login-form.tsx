...
export function LoginForm({ callbackError }: LoginFormProps) {
  ...
  const displayError = authError ?? (callbackError ? 'Une erreur est survenue lors de la connexion. Veuillez réessayer.' : null);

  return (
    <Card className="w-full max-w-sm">
      <CardHeader className="text-center">
        <CardTitle className="font-heading text-2xl font-bold">Se connecter</CardTitle>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-4">
        {displayError && (
          <p role="alert" className="rounded-xl border border-destructive bg-destructive/10 p-3 text-sm text-destructive">
            {displayError}
          </p>
        )}
